...
- 2026-07-19: Live design-fidelity screenshots are usually impractical inside an isolated feature worktree during `/run-plan`: the shared dev server on :4000/:4001 belongs to a *different* worktree (a self-check there renders unrelated code and silently passes/fails against the wrong UI), and a fresh worktree has no local `client/.env`/`server/.env` to boot its own stack. Net effect on the multi-agent-review run: all UI tasks (T-10/11/12/13) deferred live checks to structural JSX-vs-PNG comparison and reported DONE_WITH_CONCERNS; plan-verifier marked those ACs CANNOT-VERIFY. Decide up front — either accept structural-only checks, or provision dedicated ports + `.env` for the worktree before the UI waves — don't have N parallel UI agents each rediscover it, and never trust a :4000 screenshot without first confirming which worktree owns that server (T-08 hit exactly this false-negative).
...
- 2026-07-19: On a long-lived feature worktree branch, `git diff main...HEAD` is misleading for post-build verification — the merge-base is far behind `main` and the implementation is uncommitted working-tree state, so the three-dot diff omits every new feature file (on the multi-agent-review run, plan-verifier saw an "empty" feature diff and nearly under-reported coverage). Use `git status --short` + the working tree as ground truth on these branches, not the committed `main...HEAD` diff.
...
- 2026-07-19: In a fresh git worktree, `reviewer-core/node_modules` can be absent (no-hoisting monorepo — each package installs separately), which makes `cd server && pnpm exec tsc --noEmit` emit ~7 phantom errors (`openai`/`zod` unresolved under `reviewer-core/src/llm/*`, cascading `TS2322` in `server/src/adapters/llm/{anthropic,openai}.ts`) that every parallel `implementer` correctly flags as "pre-existing, not mine." Fix once at the coordinator level before trusting the server tsc gate: `cd reviewer-core && pnpm install`. Confirmed on the multi-agent-review `/run-plan` — 3 implementers independently reported the identical phantom set, and the gate went green immediately after the install.
