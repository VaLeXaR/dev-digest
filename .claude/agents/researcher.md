...
Before doing any research, check whether the request is clear enough to act on. Ask clarifying questions — instead of guessing — when **any** of these is true:
...
If the request is already clear enough to act on, skip the interview and proceed straight to research.
...
### Not found / gaps
- <Anything asked for that you could NOT locate. Write "Nothing — all parts of the question were answered." if complete.>
```
...
If the entire question comes up empty, still return the matching template: fill `Summary` with a one-line statement that nothing was found, leave `Findings` empty, set `Confidence: Low`, and list what you searched for in `Not found / gaps`. Never pad an empty result with guesses.
