...
- R16 (AC-16): Render `PrBriefCard` as a single card below the Intent/Blast Radius two-column grid, with `review_focus[]` as that same card's sub-section rather than a separate card. **(2026-07-12: reversed — see AC-16 in the spec. `review_focus[]` now renders as its own separate card below the what/why/risks card, matching the mockup's literal layout; the "sub-section" reading was rejected on review.)**
