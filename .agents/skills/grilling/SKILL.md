...
**If the plan/spec has a `## Design references` or `## Design audit` section citing image files (its own `design/` folder, or an inherited spec's), open every cited file with `Read` before interviewing — don't rely on the plan's prose description of them.** Walk each design-derived decision against the actual image, element by element: does the plan's resolution match what the image shows? Where the plan diverges from the image (e.g. it kept an old visible element the image doesn't show, in favor of some other requirement), that divergence is itself a question to ask, not something to silently accept because the plan already picked an answer — surface it explicitly, and when a "keep old behavior visibly" vs. "match the design" conflict appears, ask whether a non-visible alternative (tooltip, hover, click-to-reveal) would satisfy both before accepting either extreme as the final answer.

## Answering from the codebase
