  ...
  forbidden: [
    ...
    },
    {
      ...
      comment:
        ...
        'Exception: adapters/git/diff-parser (`parseUnifiedDiff`) is a PURE text parser — no I/O, ' +
        'its only import is a `type` from @devdigest/shared — mis-filed under adapters/; ' +
        'eval/service + review-diff/service use it. Clean fix: relocate it to a pure-util location ' +
        '(e.g. _shared) and delete this pathNot.',
      severity: 'error',
      from: {
        path: 'src/modules/[^/]+/(service|run-executor)[^/]*\\.ts$',
        pathNot: 'src/modules/repo-intel/',
      },
      to: { path: 'src/adapters/', pathNot: 'src/adapters/git/diff-parser' },
    },
    {
    ...
    },
    {
      ...
      comment:
        'Drizzle/db schema queries belong in modules/*/repository*. DRIFT (warn): several files ' +
    ...
    },
    {
      ...
      comment:
        ...
        'eval/service → reviews/{constants,helpers,diff-loader} (accepted Application-layer imports).',
      severity: 'warn',
